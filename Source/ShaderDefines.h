...
namespace Shaders
{
    ...
    static const uint32_t s_RAINBOW_COLORS_COUNT                 = 8;
    static const float4 s_RAINBOW_COLORS[s_RAINBOW_COLORS_COUNT] = {
    ...
    // NOTE: By default all textures in glTF are in sRGB color space, so we convert all textures firstly in linear RGB color space and then
    // apply in the end gamma correction.
    ...
    // Input encoded float2, [0, 1] on each component.
...
#define LIGHT_CLUSTER_SUBDIVISON_SIZE_Z 8u
    ...
    static const uint3 s_LIGHT_CLUSTER_SUBDIVISONS = uint3(16, 9, 24);
