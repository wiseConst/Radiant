...
#define MAX_SPOT_LIGHT_COUNT 256
    ...
    {
        float3 Direction;
        uint32_t Color;  // NOTE: rgb=Packed color, (a=bCastShadows)
        float Intensity;

        // LightSize(it can be in UV space, depends). Used for PCSS.
        // Controls the overall softness of the scene's shadows.
        // Larger lights will result in softer shadows.
        float Size;
...
#include <../Assets/Shaders/kernels.slang>
    ...
    namespace Shaders
    {
        ...
        [vk::binding(s_BINDLESS_COMBINED_IMAGE_SAMPLER_BINDING,
                     0)] Sampler2DArray Texture_Array_Heap[s_MAX_BINDLESS_COMBINED_IMAGE_SAMPLERS];
        ...
        float2 VogelDiskSample(const uint sampleIndex, const uint samplesCount, const float phi)
        {
            ...
            const float r     = sqrt(sampleIndex + 0.5f) / sqrt(samplesCount);
        ...
        float InterleavedGradientNoise(const float2 screenPosition)
        {
            static const float3 magic = float3(0.06711056f, 0.00583715f, 52.9829189f);
            return frac(magic.z * frac(dot(screenPosition, magic.xy)));
        }
